...
-   Q1. Consider the use case of `Compute basic/advance result`, **HOW** is this functionality made available to both JiBaBoom and Adminstrator _(Hint: They are different)_? `(Jibaboom can only insert basic/advance data whereas Administrator can only view basic/advance data. Both JiBaBoom and Administrator are able to compute basic result. JiBaBoom can have access to all the data including editing, computing and viewing all the data and results. However, Administrator can only view the data but has no access to it. JiBaboom as a system is able to compute basic/advance result from the backend through the API while Administrator is able to compute basic/advance result by clicking a link/button in the frontend(HTTP website) provided by the system.`

-   Q2. Consider the use case of `Insert basic/advance data`. **HOW** is the data stored? `(After inserting basic/advance data using SQL query, backend server will send request to the database. It will then serve an API built on the REST client and recieve the request, data is being stored in the database.`
...
3. How should the frontend display data from the database? `(Query result is returned to the database server through the API, the database will then send the requested data back to the frontend)`
4. How does the frontend communicate with backend? `(Frontend communicates with backend by talking to the server (network or localhost). Frontend will talk to REST API through the HTTP`
...
```
Answer: 
1. Firstly, client makes a GET request to the server and creates an URL that contains the information about the client's request. 
2. Server is listening for the request from the client.
3. Event listeners are activated and runs in the middleware. 
4. The request will get more information from the database query.
5. Database query is executed and will be sent back to the server, and the response is sent back to the client.  
